...
- **눈 시선 추적**: MediaDevices API와 Canvas를 사용하여 실시간으로 사용자의 시선 위치를 추적
- **자동 스크롤**: 화면 상단을 바라보면 위로, 하단을 바라보면 아래로 자동 스크롤
- **사용자 정의 설정**: 스크롤 속도와 감도 영역을 조절 가능
- **눈 위치 보정**: 정확한 추적을 위한 캘리브레이션 기능
- **직관적인 UI**: 아름다운 인터페이스로 쉽게 제어
...
5. **디버그 모드 사용 (선택사항)**
   ...
   - "디버그 모드" 체크박스를 활성화
   - 메인 시선 추적을 시작하면 디버그 패널에 실시간 정보 표시
   - **좌우 반전**: 자연스러운 시선 관찰을 위해 카메라 화면 반전
   - **자동 줌**: 눈 영역을 자동으로 찾아 확대해서 표시
   - **줌 조절**: 슬라이더로 1x ~ 4x 줌 레벨 조절 가능
   - **눈 중앙 맞추기**: 버튼을 눌러 눈 위치를 다시 찾아서 중앙에 맞추기
   - 녹색 사각형: 눈 영역 감지 표시
   - 빨간 선/점: 시선 방향 시각화
   - 실시간 메트릭: 밝기 차이, 스크롤 방향, 가속도, 눈 추적 상태 확인
